...
export function OverviewPage() {
  ...
  const kpis = [
    { label: 'This month', emoji: '💸', value: currency.format(thisTotal), tint: 'from-coral/15 to-peach/20', href: '/analytics?tab=category&preset=thisMonth' },
    { label: 'Last month', emoji: '📆', value: currency.format(lastTotal), tint: 'from-lavender/15 to-sky/15', href: '/analytics?tab=category&preset=lastMonth' },
    { label: 'Δ vs last', emoji: delta >= 0 ? '↗️' : '↘️', value: `${delta.toFixed(1)}%`, icon: delta >= 0 ? ArrowUp : ArrowDown, tint: delta >= 0 ? 'from-rose/15 to-coral/10' : 'from-mint/15 to-sage/15', href: '/analytics?tab=trend&preset=thisYear' },
    { label: 'Transactions', emoji: '🧾', value: String(thisMonth.length), tint: 'from-sky/15 to-lavender/10', href: '/expenses?preset=thisMonth&from=overview' },
  ]
