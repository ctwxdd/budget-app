...
function CardDialog({ open, onOpenChange, card }: { open: boolean; onOpenChange: (open: boolean) => void; card: CardRow | null }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange} title={card ? 'Edit card' : 'Add card'} description="Save card options to the Cards tab in Google Sheets." mobileBottomSheet>
    <form onSubmit={submit} className="grid gap-4 sm:grid-cols-2">
      ...
      <div className="sticky bottom-0 -mx-5 -mb-5 flex flex-col-reverse gap-2 border-t bg-card p-4 sm:col-span-2 sm:-mx-7 sm:-mb-7 sm:flex-row sm:justify-end"><Button type="button" variant="outline" onClick={() => onOpenChange(false)}>Cancel</Button><Button type="submit" disabled={saving}>{saving ? 'Saving...' : (card ? 'Save changes' : 'Add card')}</Button></div>
    </form>
  </Dialog>
}
