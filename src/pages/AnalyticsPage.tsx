...
export function AnalyticsPage() {
  ...
  return <div className="space-y-5 md:space-y-6"><ExpenseFilterBar filters={filters} onChange={setFilters} /><Tabs value={tab} onChange={setTab} tabs={[
    { value: 'category', label: 'By Category', content: tab === 'category' ? <Card><CardHeader><CardTitle>Spending by category</CardTitle><CardDescription>Tap a slice or category to focus the breakdown.</CardDescription></CardHeader><CardContent>{category.length ? <CategoryBreakdown rows={category} /> : <EmptyChart />}</CardContent></Card> : null },
