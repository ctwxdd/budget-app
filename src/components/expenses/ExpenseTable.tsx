...
function ExpenseCard({ expense, onEdit, onRemove, selected, selectionMode, onToggleSelected, onEnterSelectionMode }: { expense: Expense; onEdit: (expense: Expense) => void; onRemove: (expense: Expense) => void; selected: boolean; selectionMode: boolean; onToggleSelected: (expense: Expense) => void; onEnterSelectionMode: (expense: Expense) => void }) {
  ...
  >
    ...
    <div className="mt-3 flex min-w-0 flex-wrap items-center gap-2"><div className="min-w-0 max-w-[45%]"><ColorBadge value={expense.category || 'Uncategorized'} /></div><div className="min-w-0 max-w-[55%]"><ColorBadge value={expense.paymentMethod || 'Unknown'} variant="payment" /></div>{!selectionMode && <div className="ml-auto"><Button variant="ghost" size="icon" aria-label="Expense actions" onClick={(event) => { event.stopPropagation(); setOpen((value) => !value) }}><MoreHorizontal className="h-5 w-5" /></Button></div>}</div>
    {open && <div className="absolute bottom-14 right-4 z-10 grid min-w-32 gap-1 rounded-2xl border bg-card p-1 shadow-lg" onClick={(event) => event.stopPropagation()}><Button variant="ghost" className="justify-start" onClick={() => { setOpen(false); onEdit(expense) }}><Pencil className="h-4 w-4" />Edit</Button><Button variant="ghost" className="justify-start text-destructive hover:text-destructive" onClick={() => { setOpen(false); onRemove(expense) }}><Trash2 className="h-4 w-4" />Delete</Button></div>}
  </div>
}
